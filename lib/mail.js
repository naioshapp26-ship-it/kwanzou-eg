...
async function sendAdminAlertEmail({ subject, text, html }) {
  const admin = (process.env.ADMIN_EMAIL || process.env.SMTP_USER || '').trim();
  if (!admin) return { ok: false, error: 'no_admin_email' };
  const mailStatus = getMailStatus();
  if (!mailStatus.configured) return { ok: false, error: 'mail_not_configured' };

  const resend = getResendConfig();
  const smtp = getSmtpConfig();
  const from = resend.configured ? resend.from : smtp.from;

  try {
    if (resend.configured) {
      return await sendViaResend({ to: admin, from, subject, html, text, apiKey: resend.apiKey });
    }
    return await sendViaSmtp({ to: admin, from, subject, html, text, smtp });
  } catch (err) {
