...
const { repairCategoryProductLinks } = require('./category-links');
...
async function prepareStoreForSave(data, pool) {
  ...
  repairCategoryProductLinks(stamped);
...
async function runStartupMigration(current, pool) {
  ...
  // Always re-link products ↔ categories (idempotent).
  {
    if (!changed) data = JSON.parse(JSON.stringify(data));
    const repaired = repairCategoryProductLinks(data);
