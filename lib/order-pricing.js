...
// Only accept a color that the product actually offers; store the canonical entry.
function sanitizeColor(submitted, product) {
  const colors = Array.isArray(product?.colors) ? product.colors : [];
  if (!colors.length || !submitted) return null;
  const keys = [submitted.name, submitted.nameAr, submitted.hex]
    .map(normalizeColorKey)
    .filter(Boolean);
  const match = colors.find(c =>
    keys.includes(normalizeColorKey(c.name)) ||
    keys.includes(normalizeColorKey(c.nameAr)) ||
    keys.includes(normalizeColorKey(c.hex))
  );
...
function validateOrderPayload(store, payload) {
  ...
  for (const item of items) {
    ...
    validatedItems.push({
      ...
      color: sanitizeColor(item.color, product)
    });
  }
