...
function normalizeCategorySlugs(categories) {
  ...
  // 1) Fix Arabic / junk / alias slugs on any category.
  ...
  // 2) Children must not reuse a parent's slug (or any sibling slug).
  //    This is what made necklace/rings products "disappear" from sub-filters.
  ...
  for (const cat of categories) {
    ...
    if (!collidesWithParent && !collidesWithSibling) continue;

    const base = preferredChildSlug(cat) || `${parent.slug}-item`;
