...
/** Keyword rules to recover products after duplicate-slug merges misfiled them. */
const KEYWORD_SUBCAT_RULES = [
  {
    targetSlug: 'necklaces-drop',
    parentFamily: /^(necklaces|long-necklace)/,
    // Match "drop necklace" / "drop" in product titles (not category field).
    test: hay => /\bdrop\b/.test(hay)
  },
  {
    targetSlug: 'necklaces-queen',
    parentFamily: /^(necklaces|long-necklace)/,
    test: hay => /\bqueen\b|كوين/.test(hay)
  }
];

function isNecklaceFamilySlug(slug) {
  const s = String(slug || '');
  return s === 'long-necklace' || s.startsWith('necklaces');
}

function reassignByProductKeywords(products, categories) {
  ...
  products.forEach(p => {
    ...
    if (!isNecklaceFamilySlug(slug)) return;
    const hay = `${p.name || ''} ${p.nameAr || ''}`.toLowerCase();

    for (const rule of KEYWORD_SUBCAT_RULES) {
      const target = bySlug.get(rule.targetSlug);
      if (!target) continue;
      if (slug === rule.targetSlug) continue;
      if (!rule.parentFamily.test(slug) && slug !== rule.targetSlug) continue;
      if (!rule.test(hay)) continue;
      p.categorySlug = target.slug;
      p.category = target.name || target.nameAr;
...
function repairProductCategoryLinks(products, categories) {
  ...
  if (reassignByProductKeywords(products, categories)) changed = true;
  return changed;
}
