...
async function notifyNewContact(msg) {
  ...
  const email = msg.email ? ` — ${msg.email}` : '';
  return addAdminNotification({
    type: 'contact',
    title: `رسالة تواصل — ${msg.name}`,
    message: `${msg.message.slice(0, 120)}${msg.message.length > 120 ? '…' : ''}${phone}${email}`,
    meta: { contactId: msg.id, phone: msg.phone || '', email: msg.email || '', subject: msg.subject || '' }
  });
}
